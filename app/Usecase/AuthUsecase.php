...
class AuthUsecase implements AuthUsecaseInterface
{
    ...
    public function Register(Request $request)
    {
        ...
        $org_id = 0;
        if (strtolower($request['role']) == 'organization admin') {
            //create organization
            $org = $this->org->storeOrganization(['name' => 'ORG' . ' ' . $request['username']]);
        ...
        //get role by name
        $role = $this->roleEloq->getRoleByName(strtolower($request['role']));
        if ($role == null) {
            return ApiResponse::errorResponse('Role' . ' ' . $request['role'] . ' ' . "Doesn't Exists", '', 404);
        }

        //save to user
        $user = $this->eloquentUser->storeUser($request);
        //save to user detail
